...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF6F0] pb-24">
      {/* Header */}
      <div className="bg-white border-b border-gray-100 px-4 py-4 flex items-center justify-between max-w-lg mx-auto">
        <div className="flex items-center gap-2">
          <span className="text-pink-500 text-xl font-bold">✦</span>
          <span className="text-lg font-bold text-gray-900">Můj profil</span>
        </div>
        {user.premium && (
      ...
      <div className="max-w-lg mx-auto px-4 pt-6 space-y-4">
        ...
        <div className="card p-6 flex flex-col items-center text-center">
          <div className="w-28 h-28 rounded-full overflow-hidden mb-4 ring-4 ring-pink-100">
            {photo ? (
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            <span className="bg-gray-100 text-gray-600 text-xs font-medium px-3 py-1 rounded-full">{genderLabel}</span>
            {user.city && (
        ...
        {personologyText && (
          <div className="card p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-bold text-gray-900">Tvůj otisk osudu</h2>
            </div>
            <p className="text-gray-600 text-sm leading-relaxed line-clamp-4">{personologyText}</p>
          </div>
        )}
        ...
        {user.bio && (
          <div className="card p-5">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-lg">💬</span>
              <h2 className="font-bold text-gray-900">O mně</h2>
            </div>
        ...
        {user.hobbies && user.hobbies.length > 0 && (
          <div className="card p-5">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-lg">✨</span>
              <h2 className="font-bold text-gray-900">Zájmy</h2>
            </div>
            <div className="flex flex-wrap gap-2">
              {user.hobbies.map((h, i) => (
                <span key={i} className="bg-purple-50 text-purple-700 text-xs font-medium px-3 py-1.5 rounded-full">
                  {h}
                </span>
              ))}
            </div>
        ...
        {(user.occupation || user.education) && (
          <div className="card p-5">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-lg">📋</span>
              <h2 className="font-bold text-gray-900">Detaily</h2>
            </div>
        ...
        {!user.premium && (
          <Link href="/premium" className="block card p-5 bg-gradient-to-r from-purple-500 to-pink-500 text-white hover:opacity-95 transition-opacity">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-bold text-base mb-0.5">✨ Odemknout Cosmatch+</p>
                <p className="text-white/80 text-sm">Neomezené swipy, kdo tě lajkoval a víc</p>
              </div>
              <svg viewBox="0 0 24 24" className="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5">
        ...
        <p className="text-center text-gray-300 text-xs pb-2">cosmatch.cz © 2025</p>
      </div>
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm">
            <h3 className="text-xl font-bold text-gray-900 mb-2 text-center">Odhlásit se?</h3>
            <p className="text-gray-500 text-sm text-center mb-6">Vrátíš se na přihlašovací obrazovku.</p>
