...
function Avatar({ name }: { name: string }) {
  ...
  return (
    <div className={`w-14 h-14 rounded-full bg-gradient-to-br ${AVATAR_COLORS[idx]} flex items-center justify-center font-bold text-white text-xl shrink-0`}>
    ...
    <div className="min-h-screen">
      <nav className="flex items-center justify-between px-6 py-4 max-w-2xl mx-auto">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="font-bold">Cosmatch</span>
        </Link>
        <Link href="/discover" className="btn-secondary text-sm py-2 px-4">
          {lang === 'cs' ? '🔍 Objevovat' : '🔍 Discover'}
        </Link>
      </nav>

      <div className="max-w-2xl mx-auto px-4 pb-10">
        <h1 className="text-2xl font-bold mb-6">{lang === 'cs' ? 'Tvoje shody' : 'Your Matches'}</h1>
        ...
        {displayList.length === 0 ? (
          <div className="card p-12 text-center">
            <div className="text-4xl mb-4">{tab === 'matches' ? '💜' : tab === 'mutual' ? '🌌' : '👀'}</div>
            ...
            {tab === 'matches' && (
              <Link href="/discover" className="btn-primary inline-block mt-4">
                {lang === 'cs' ? 'Objevovat' : 'Discover'}
              </Link>
            )}
          </div>
          ...
          <div className="space-y-3">
            {tab === 'mutual' && (
              <div className="card p-4 bg-cyan-500/10 border-cyan-400/30 mb-4">
            ...
            {displayList.map(p => {
              ...
              return (
                <div key={p.id} className="card p-4 flex items-center gap-4 hover:bg-white/15 transition-all">
                  <Avatar name={p.name} />
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {c && <CompatBadges compat={c} lang={lang} />}
                    </div>
                  </div>
                  {c && <ScoreRing score={c.score} />}
                </div>
              )
            })}
          </div>
