...
function BottomNav({ active }: { active: string }) {
 ...
 <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor"> <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/> </svg> )},
 ...
 return (
 <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 z-30"> <div className="flex max-w-lg mx-auto"> {items.map(item => (
...
export default function PremiumPage() {
 ...
 <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-sm border-b border-gray-100"> <div className="flex items-center gap-3 px-4 py-3 max-w-lg mx-auto"> <Link href="/discover" className="p-2 rounded-xl hover:bg-gray-100 transition-colors"> <svg viewBox="0 0 24 24" className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" strokeWidth="2"> <path d="M19 12H5M12 19l-7-7 7-7"/> </svg> </Link> <div className="flex items-center gap-2"> <span className="text-xl"></span> <span className="font-bold text-gray-900">Cosmatch Premium</span> </div> </div> </div> <div className="max-w-lg mx-auto px-4 py-8"> {/* Premium badge */}
 {user.premium && (
 <div className="card p-4 mb-6 bg-gradient-to-r from-pink-50 to-purple-50 border-pink-200 text-center"> <div className="text-2xl mb-1"></div> <p className="font-semibold text-pink-700">Jsi Premium člen/ka!</p> <p className="text-sm text-gray-500 mt-1">Těšíš se z plného přístupu ke všem funkcím.</p> </div> )}

 {/* Hero text */}
 <div className="text-center mb-8"> <div className="inline-flex items-center gap-2 bg-pink-50 border border-pink-200 rounded-full px-4 py-1.5 text-pink-600 text-sm font-medium mb-4"> Odemkni plný potenciál
 </div> <h1 className="text-2xl font-bold text-gray-900 mb-2"> Najdi svůj dokonalý match
 </h1> <p className="text-gray-500 text-sm leading-relaxed"> Premium ti dá přístup ke všem profilům, pokročilým filtrům a uvidíš kdo tě lajknul dřív, než ty je.
 </p> </div> {/* Pricing cards */}
 <div className="flex flex-col gap-4 mb-8"> {PLANS.map(plan => (
 <div
 key={plan.id}
 ...
 <div className="flex items-start justify-between mb-3"> <div> <h2 className="font-bold text-gray-900 text-lg">{plan.name}</h2> {plan.subtext && (
 <p className="text-xs text-pink-600 font-medium mt-0.5">{plan.subtext}</p> )}
 </div> <div className="text-right"> <span className="text-2xl font-bold text-gray-900">{plan.price}</span> <span className="text-gray-400 text-sm ml-1">{plan.period}</span> </div> </div> <ul className="space-y-1.5 mb-4"> {plan.features.map((f, i) => (
 <li key={i} className="flex items-center gap-2 text-sm text-gray-600"> <span className="text-pink-500 text-xs"></span> {f}
 </li> ))}
 </ul> <button
 ...
 { icon: '↩', text: 'Zrušení kdykoliv' },
 { icon: '', text: 'EU faktura s DPH' },
 ].map((b, i) => (
 <div key={i} className="text-center"> <div className="text-xl mb-1">{b.icon}</div> <p className="text-xs text-gray-500">{b.text}</p> </div> ))}
 </div> {/* FAQ */}
 <div className="card p-5"> <h3 className="font-semibold text-gray-900 mb-4">Časté otázky</h3> <div className="space-y-4"> {[
 ...
 <div key={i}> <p className="text-sm font-medium text-gray-800">{faq.q}</p> <p className="text-sm text-gray-500 mt-1">{faq.a}</p> </div> ))}
 </div> </div> </div> <BottomNav active="premium" /> </div> )
}
