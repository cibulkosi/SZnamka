...
const TRINITIES = [
  { name: 'Trojice nezávislosti', numbers: [1, 5, 7], color: 'amber', desc: 'Vlastní směr, svoboda, mentální hloubka. Čísla, která jdou vlastní cestou.' },
  { name: 'Trojice stability', numbers: [2, 4, 8], color: 'emerald', desc: 'Citlivost, struktura, moc. Čísla, která budují trvalé hodnoty.' },
  { name: 'Trojice tvořivosti', numbers: [3, 6, 9], color: 'pink', desc: 'Vyjádření, péče, soucit. Čísla, která tvoří a léčí svět kolem sebe.' },
] as const

export default function NumericMrizkaPage() {
  return (
      ...
      <article className="max-w-2xl mx-auto px-6 pt-16 pb-24">
        ...
        <section className="mb-16">
          ...
          <div className="space-y-4 mb-6">
            {TRINITIES.map((t) => (
              <div key={t.name} className={`bg-${t.color}-50 border border-${t.color}-200 rounded-2xl p-6`}>
                <p className={`eyebrow text-${t.color}-700 mb-3`}>{t.name}</p>
                ...
                <p className="text-gray-700 text-[0.95rem] leading-relaxed">{t.desc}</p>
              </div>
            ))}
          </div>
        ...
        {/* Význam jednotlivých čísel (chybí) */}
        <section className="mb-16">
