...
// Voice prompt upload — Phase 2 Hinge voice notes
// Accepts: audio/webm, audio/mp4, audio/mpeg, audio/ogg, audio/aac
// Max:     5 MB / typically 1-60 seconds duration
// Storage: R2 bucket key voice/<userId>/<uuid>.<ext>
...
// Allowed audio MIME types (browser MediaRecorder + native audio)
...
function normaliseType(t: string): string {
  // Strip "codecs=..." suffix for matching
...
serve(async (req) => {
  ...
  try {
    ...
    const base = normaliseType(file.type);
    const ext = ALLOWED_TYPES[base] ?? ALLOWED_TYPES[file.type];
    if (!ext) {
      return new Response(JSON.stringify({
        error: `Unsupported audio type: ${file.type}. Allowed: webm, mp4/m4a, mp3, aac, ogg.`,
      }), { status: 400, headers: cors });
    }

    if (file.size > 5 * 1024 * 1024) {
      return new Response(JSON.stringify({ error: 'Max 5 MB (typically 30–60 s audio)' }),
        { status: 400, headers: cors });
    }

    const key = `voice/${userId}/${crypto.randomUUID()}.${ext}`;
    const buffer = new Uint8Array(await file.arrayBuffer());
    const url = await uploadToR2(buffer, key, base);
